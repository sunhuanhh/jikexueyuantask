...
var maxChar = Object.keys(result).sort(function(a, b) {
    return result[a].length < result[b].length;  //降序 return result[b].length - result[a].length;
})[0];

document.write('出现次数最多的字母是：' + maxChar + '<br/>');
document.write('共出现了' + result[maxChar].length + '次<br/>');
document.write('它在数组中的位置是：' + result[maxChar]);
